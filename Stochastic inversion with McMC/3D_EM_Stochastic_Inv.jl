...
# sigma_true = CSV.read(
#     joinpath(data_dir, "set_1_curved_sigma_model_true.csv"), 
#     DataFrame; 
#     header=false
# ) |> Matrix;
# 
# # Add channel-wise random noise to true data
# EMnoise = zeros(size(EMdata_true));
# for t in 1:N_time
#     channel_data = EMdata_true[(t-1)*N_obs+1:t*N_obs];
#     channel_noise_std = 0.01f0 * maximum(channel_data);
#     channel_noise = rand(Normal(0, channel_noise_std), length(channel_data)); # Generate noise with the same length as true data
#     EMnoise[(t-1)*N_obs+1:t*N_obs] .= channel_noise; # Add noise to true data
# end
# EMdata_true += EMnoise; # Contaminate observed data
# 
# # Load true Gaussian Process hyper-parameters
# GP_hyper_true = CSV.read(
#     joinpath(data_dir, "set_1_curved_hyper_param.csv"), 
#     DataFrame; 
#     header=false
# ) |> Matrix;
